...
public class EmployersLobby {
	...
	public void employersMenu(Client client, List<Job> jobs) {
		...
		for (boolean valid = false; !valid;) {
			...
			/*
			 * case 3:{ shortListApplicant(client,jobs); valid=true; }
			 */
	...
	/*
	 * public void shortListApplicant(Client client,List<Job> jobs) { Scanner in=new
	 * Scanner(System.in); SignOut signout=new SignOut(); Employer
	 * employer=(Employer)client; int count=0,choice;
	 * 
	 * for(Job jobsPosted:jobs) {
	 * if((jobsPosted.employerId).equals(employer.getEmployerId())) {
	 * this.flag=true; for(JobSeeker applicant:jobsPosted.applicants) {
	 ...
	 * System.out.println("1.Yes\t2.No"); choice=in.nextInt(); } } }
	 * 
	 * int option; System.out.println("\n1.Menu\n2.Logout"); for(boolean
	 * valid=false;!valid;) { option=in.nextInt(); switch(option) {
	 * 
	 * case 1:{ employersMenu(client,jobs); valid=true; } break;
	 * 
	 * case 2:{ signout.signOut(); valid=true; } break;
	 * 
